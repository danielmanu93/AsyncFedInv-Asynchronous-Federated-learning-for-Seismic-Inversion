...
#        data1_set = np.float32(data1_set[str(dataname)].reshape([data_dim[0],data_dim[1],in_channels]))
        data1_set=np.load(filename_seis+'.npy')
        for k in range (0,in_channels):
            # For .mat data change the dimention [h, w, c] --> [c, h, w]
            ...
            data11_set     = block_reduce(data11_set,block_size=data_dsp_blk,func=np.max)
        ...
        data2_set=np.load(filename_label+'.npy')
